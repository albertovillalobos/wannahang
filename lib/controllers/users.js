...
exports.searchByUsername = function(req, res) {
  ...
  // res.json({message: 'This is the search function.'});
  var userToFind = req.params.username;
  // res.json({input: userToFind});

  User.findOne({username: userToFind}, function(err, user) {
    if(err) return next(err);
    if(!user) return res.send(404);


    User.findById(user._id, function(err2, user2) {

      if(err2) return next(err2);
      if(!user2) return res.send(404);

      // res.json(user2)
      res.send({ profile: user2.profile });
